...
class EdgeSplitter:
    ...
    def _train_test_split_homogeneous(self, p, method, probs=None, keep_connected=False):
        """
        ...
        if method == "global":
            negative_edges = self._sample_negative_examples_global(p=p, limit_samples=len(positive_edges))
        else:  # method == 'local'
            if probs is None:  # use default values if not given, by warn user
                probs = [0.0, 0.25, 0.50, 0.25]
                warnings.warn(
                    "Using default sampling probabilities (distance from source node): {}".format(probs),
                    RuntimeWarning,
                    stacklevel=2,
                )
    ...
    def _train_test_split_heterogeneous(
        ...
        if edge_attribute_threshold is None:
            positive_edges = self._reduce_graph_by_edge_type(minedges=self.minedges_set, p=p, edge_label=edge_label)
        else:
    ...
    def train_test_split(
        ...
        if not isinstance(keep_connected, (bool,)):
            raise ValueError("({}) The flag keep_connected be boolean type.".format(type(self).__name__))
    ...
    def _get_edges(self, edge_label, edge_attribute_label=None, edge_attribute_threshold=None):
        ...
        if edge_attribute_label is None or edge_attribute_threshold is None:
            ...
            edges_with_label = [e for e in all_edges if self.g.get_edge_data(*e)["label"] == edge_label]
            ...
            edges_with_label = [
                ...
                if (
                    ...
                    and datetime.datetime.strptime(self.g.get_edge_data(*e)[edge_attribute_label], "%d/%m/%Y")
    ...
    def _sample_negative_examples_by_edge_type_local_dfs(
        ...
        if probs is None:
            probs = [0.0, 0.25, 0.50, 0.25]
            warnings.warn(
                "Using default sampling probabilities up to 3 hops from source node with values {}".format(probs)
            )
        ...
        for _ in np.arange(0, num_iter):
            ...
            target_node_distances = self._random.choice(n, len(start_nodes), p=probs) + 1
